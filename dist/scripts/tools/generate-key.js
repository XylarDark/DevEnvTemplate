...
const crypto_helpers_1 = require("../utils/crypto-helpers");
...
async function main() {
    ...
    if (options.format === 'hex') {
        // Generate hex format
        key = (0, crypto_1.randomBytes)(length).toString('hex');
    }
    else {
        // Generate base64 format (default)
        key = (0, crypto_helpers_1.generateEncryptionKey)(length);
    }
    // Validate if requested
    if (options.validate && options.format === 'base64') {
        const validation = (0, crypto_helpers_1.validateBase64Key)(key, length);
        if (!validation.valid) {
    ...
    if (options.quiet) {
        ...
        if (options.format === 'base64') {
            const expectedLength = (0, crypto_helpers_1.getExpectedBase64Length)(length);
            console.log(`Key details:`);
            console.log(`  Format: Base64 URL-safe`);
            console.log(`  Length: ${length} bytes (${expectedLength} base64 characters)`);
            console.log(`  Valid: ${(0, crypto_helpers_1.validateBase64Key)(key, length).valid ? 'Yes' : 'No'}`);
        }
        else {
            console.log(`Key details:`);
