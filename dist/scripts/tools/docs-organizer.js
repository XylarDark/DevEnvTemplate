...
const docs_organizer_1 = require("../utils/docs-organizer");
/**
 * Format file path for display
 */
function formatPath(filePath, projectRoot) {
...
async function main() {
    ...
    const projectRoot = options.projectRoot || (0, path_resolver_1.resolveProjectRoot)(currentDir) || currentDir;
    if (projectRoot !== currentDir) {
        process.chdir(projectRoot);
    }
    try {
        // Validate organization is needed
        const validation = await (0, docs_organizer_1.validateOrganization)(projectRoot);
        ...
        const config = await (0, docs_organizer_1.loadDocsConfig)(projectRoot);
        for (const filename of validation.misplacedFiles) {
            const { target, reason } = (0, docs_organizer_1.determineTargetDirectory)(filename, config, projectRoot);
        ...
        if (options.dryRun || !options.autoFix) {
            ...
            const result = await (0, docs_organizer_1.organizeDocumentation)(projectRoot, true);
        ...
        console.log('='.repeat(50) + '\n');
        const result = await (0, docs_organizer_1.organizeDocumentation)(projectRoot, false);
