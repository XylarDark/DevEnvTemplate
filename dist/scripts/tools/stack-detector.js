...
const detectorMode = requestedMode === 'fast' ? 'fast' : 'full';
const logger = (0, logger_1.createLogger)({ context: 'stack-detector' });
...
class StackDetector {
    ...
    async readFileCached(filePath, options = {}) {
        const allowMissing = options.allowMissing !== false;
        const absPath = path_1.default.isAbsolute(filePath) ? filePath : path_1.default.join(this.rootDir, filePath);
    ...
    async readJsonFile(filePath) {
        ...
        try {
            ...
            if (error instanceof SyntaxError) {
                ...
                try {
                    return JSON.parse((0, jsonc_1.stripJsonComments)(content));
                }
                catch {
                    throw (0, error_helpers_1.createJsonParseError)(error, absPath);
                }
            }
            throw error;
        }
     ...
     */
    async detectUnrealProject() {
        try {
            const entries = await fs_1.promises.readdir(this.rootDir, { withFileTypes: true });
            const hasUProject = entries.some(e => e.isFile() && e.name.toLowerCase().endsWith('.uproject'));
            if (!hasUProject) {
                return;
            }
            ...
            this.stack.stackHints.push('Unreal-like repo: .uproject at repository root. Ensure .cursor/rules/21-unreal-engine.mdc and 22-unreal-editor-ui.mdc are present; see docs/templates/unreal/README.md.');
     ...
     */
    async detectUnityProject() {
        try {
            const versionRelPaths = [path_1.default.join('ProjectSettings', 'ProjectVersion.txt')];
            const entries = await fs_1.promises.readdir(this.rootDir, { withFileTypes: true });
            for (const entry of entries) {
                ...
                versionRelPaths.push(path_1.default.join(entry.name, 'ProjectSettings', 'ProjectVersion.txt'));
            }
            let foundRel = null;
            for (const rel of versionRelPaths) {
                try {
                    await fs_1.promises.access(path_1.default.join(this.rootDir, rel));
                    foundRel = rel;
                    break;
                }
                catch {
            ...
            this.stack.stackHints.push(`Unity-like repo: ${foundRel.replace(/\\/g, '/')}. Ensure .cursor/rules/23-unity-csharp.mdc is present; see docs/templates/unity/README.md.`);
    ...
    async detectDotNet() {
        try {
            ...
            if (csprojFiles.length > 0) {
                ...
                if (csprojContent.includes('EntityFramework') ||
    ...
    async findFiles(pattern) {
        ...
        try {
            const files = await fs_1.promises.readdir(this.rootDir);
    ...
    async detectFrameworks() {
        ...
        for (const configFile of nextConfigFiles) {
            try {
                await fs_1.promises.access(path_1.default.join(this.rootDir, configFile));
                this.stack.configurations.push({
                    type: 'nextjs',
                    configFile,
                });
                ...
                const dirs = [];
                try {
                    await fs_1.promises.access(path_1.default.join(this.rootDir, 'app'));
                    dirs.push('app');
                ...
                try {
                    await fs_1.promises.access(path_1.default.join(this.rootDir, 'pages'));
                    dirs.push('pages');
                ...
                if (content.includes('express()') ||
                    ...
                    if (this.stack.frameworks.type === 'vanilla') {
                        ...
                        this.stack.frameworks = {
                            ...
                            dirs: [path_1.default.dirname(file) || '.'],
                        };
                    }
                    break;
                }
    ...
    async detectPrisma() {
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, 'prisma', 'schema.prisma'));
            ...
            try {
                await fs_1.promises.access(path_1.default.join(this.rootDir, 'schema.prisma'));
     ...
     */
    async findConfigFile(base) {
        for (const extension of ['ts', 'mts', 'cts', 'js', 'mjs', 'cjs', 'json']) {
            const candidate = `${base}.${extension}`;
            try {
                await fs_1.promises.access(path_1.default.join(this.rootDir, candidate));
                return candidate;
            }
            catch {
     ...
     */
    async containsTestFile(dir, depth = 4) {
        if (depth < 0)
            return false;
        let entries;
        try {
        ...
        for (const subdirectory of subdirectories) {
            if (await this.containsTestFile(path_1.default.join(dir, subdirectory), depth - 1))
                return true;
        }
        return false;
    }
    async detectTesting() {
        ...
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, 'tests'));
            this.stack.quality.testing = true;
            this.stack.files.key_patterns.push('tests/ (test directory)');
        }
        catch (error) {
            try {
                await fs_1.promises.access(path_1.default.join(this.rootDir, '__tests__'));
                this.stack.quality.testing = true;
                ...
                for (const dir of testDirs) {
                    if (await this.containsTestFile(path_1.default.join(this.rootDir, dir))) {
                        this.stack.quality.testing = true;
        ...
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, 'pytest.ini'));
            ...
            else if (this.requirementsContent &&
    ...
    async detectLinting() {
        ...
        for (const file of eslintFiles) {
            try {
                await fs_1.promises.access(path_1.default.join(this.rootDir, file));
        ...
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, 'ruff.toml'));
            ruffDetected = true;
    ...
    async detectCI() {
        ...
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, '.github', 'workflows'));
    ...
    async detectSecretsHygiene() {
        ...
        for (const candidate of templateCandidates) {
            const candidatePath = path_1.default.join(this.rootDir, candidate);
            try {
        ...
        const hasCompleteHygiene = secrets.envTemplate.present &&
            secrets.envIgnored &&
    ...
    async collectWorkflowFiles() {
        const workflowDir = path_1.default.join(this.rootDir, '.github', 'workflows');
        const yamlFiles = [];
        try {
    ...
    async saveReport(report) {
        ...
        const reportPath = path_1.default.join(devenvDir, 'stack-report.json');
        await fs_1.promises.writeFile(reportPath, JSON.stringify(report, null, 2), 'utf8');
        if (!this.quiet) {
            logger.info(`Stack report saved to ${reportPath}`);
        }
    }
    async detectCursorRules() {
        const cursorRulesDir = path_1.default.join(this.rootDir, '.cursor', 'rules');
        try {
            await fs_1.promises.access(cursorRulesDir);
            this.stack.cursorRules.present = true;
            // Read all .mdc files
            const entries = await fs_1.promises.readdir(cursorRulesDir, { withFileTypes: true });
            const mdcFiles = [];
            for (const entry of entries) {
            ...
            const stackFiles = [];
            const retiredAlwaysOnFiles = [];
            const projectSpecificFiles = [];
            for (const file of mdcFiles) {
                if (cursor_rules_adapter_1.STACK_SCOPED_FILES.includes(file)) {
                    stackFiles.push(file);
                }
                else if (cursor_rules_adapter_1.RETIRED_ALWAYS_ON_FILES.includes(file)) {
    ...
    assignProfiles() {
        ...
        const packageManager = typeof rawPackageManager === 'string' ? rawPackageManager.toLowerCase() : '';
        const nodeSignals = [
            'node.js',
            'node',
            'react',
        ...
        const hasNodeSignals = techNames.some(name => nodeSignals.includes(name)) ||
            manifestTechs.some(name => nodeSignals.includes(name)) ||
            nodePackageManagers.includes(packageManager);
        const hasPythonSignals = techNames.some(name => pythonSignals.includes(name)) ||
...
if (require.main === module) {
    ...
    detector
        ...
        .catch(error => {
        if (jsonOutput) {
            process.stderr.write(JSON.stringify({ error: 'Stack detection failed', message: error.message }) + '\n');
        }
        else {
