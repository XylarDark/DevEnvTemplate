...
const logger = (0, logger_1.createLogger)({ context: 'gap-analyzer' });
...
if (!parsedMode) {
    ...
    if (modeIndex !== -1 &&
        ...
        parsedMode = cliArgs[modeIndex + 1];
    }
}
...
const analyzerMode = parsedMode === 'fast' ? 'fast' : 'full';
...
class GapAnalyzer {
    ...
    async analyze() {
        logger.info('Starting gap analysis against development best practices');
        // Load stack report
        try {
            const stackReportPath = path_1.default.join(this.rootDir, '.devenv', 'stack-report.json');
            const content = await fs_1.promises.readFile(stackReportPath, 'utf8');
            this.stack = JSON.parse(content);
            ...
            const stackLanguageProfile = this.stack.languageProfile;
        ...
        if (this.isFastMode()) {
            logger.info('Fast mode enabled: skipping documentation, performance, accessibility, Docker, ' +
        ...
        if (this.isFastMode()) {
            ...
            this.gaps.push({
                ...
                description: 'Fast mode skips documentation, performance, accessibility, Docker, environment, ' +
  ...
  "compilerOptions": {
    ...
    async analyzeDocumentation() {
        ...
        try {
            const readmePath = path_1.default.join(this.rootDir, 'README.md');
            const readme = await fs_1.promises.readFile(readmePath, 'utf8');
            const requiredSections = ['Installation', 'Usage', 'Contributing', 'License'];
            const missingSections = requiredSections.filter(section => !readme.toLowerCase().includes(section.toLowerCase()));
...
`,
    ...
    async analyzeDocker() {
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, 'Dockerfile'));
            // Check for .dockerignore
            try {
                await fs_1.promises.access(path_1.default.join(this.rootDir, '.dockerignore'));
            }
            catch {
                this.gaps.push({
                    category: 'docker',
...
dist`,
            ...
            const dockerfile = await fs_1.promises.readFile(path_1.default.join(this.rootDir, 'Dockerfile'), 'utf8');
    ...
    async analyzeEnvironment() {
        ...
        try {
            const gitignore = await fs_1.promises.readFile(path_1.default.join(this.rootDir, '.gitignore'), 'utf8');
...
echo "npm run lint && npm run format:check" > .husky/pre-commit`,
    ...
    async checkAgentInstructions() {
        ...
        if (content === null) {
            this.gaps.push({
                ...
                severity: 'high',
                title: 'No AGENTS.md',
                description: 'AGENTS.md is the cross-tool standard for agent instructions, read by Cursor, Claude Code, Codex, and Gemini CLI. Without it, every agent starts by guessing how this project works.',
                impact: 'Agents infer commands and conventions, and infer them wrong',
                recommendation: 'Add AGENTS.md at the repository root stating the stack, the exact commands to run, the directory layout, and the conventions that matter',
                effort: 'medium',
                files: ['AGENTS.md'],
            });
            return;
        ...
        const hasCommands = /```|`(npm|npx|yarn|pnpm|make|cargo|go|python|dotnet|pip|uv) /.test(content);
        if (!hasCommands) {
            this.gaps.push({
                category: 'agent-context',
                severity: 'medium',
                title: 'AGENTS.md Contains No Commands',
                description: 'AGENTS.md exists but names no command an agent can run. Agents execute what this file lists; prose about intentions is not executable.',
                impact: 'Agents guess build and test commands instead of using yours',
                recommendation: 'List the exact commands for building, testing, and linting, and note any argument conventions (for example that npm needs `--` before script flags)',
                effort: 'low',
                files: ['AGENTS.md'],
            });
        }
        ...
        if (lineCount > 400) {
            this.gaps.push({
                ...
                recommendation: 'Keep AGENTS.md under roughly 200 lines. Move procedural detail into skills under .agents/skills/ and file-specific detail into glob-scoped rules',
                effort: 'medium',
                files: ['AGENTS.md'],
            });
        }
     ...
     */
    async checkAlwaysApplyBudget() {
        let entries;
        try {
            entries = await fs_1.promises.readdir(path_1.default.join(this.rootDir, '.cursor', 'rules'));
        }
        catch {
            return;
        }
        ...
        for (const entry of entries) {
            ...
            const content = await this.readFileOrNull(path_1.default.join('.cursor', 'rules', entry));
            if (content === null) {
                continue;
            }
        ...
        if (totalLines > 200) {
            this.gaps.push({
                ...
                recommendation: 'Move universal facts into AGENTS.md, procedural knowledge into .agents/skills/, and file-specific guidance into glob-scoped rules, then set alwaysApply: false',
                effort: 'medium',
     ...
     * one is current.
     */
    async checkShimDrift() {
        if (!(await this.fileExists('AGENTS.md'))) {
            return;
        }
        const shims = [
            { file: 'CLAUDE.md', tool: 'Claude Code' },
            { file: path_1.default.join('.github', 'copilot-instructions.md'), tool: 'GitHub Copilot' },
        ];
    ...
    async checkMcpConfig() {
        for (const candidate of [path_1.default.join('.cursor', 'mcp.json'), '.mcp.json']) {
            ...
            const hasInlineSecret = /"[A-Za-z0-9_]*(?:KEY|TOKEN|SECRET|PASSWORD|CREDENTIALS?)"\s*:\s*"(?!\$\{)[^"]{8,}"/i.test(content);
            if (hasInlineSecret) {
                this.gaps.push({
                    category: 'security',
                    severity: 'high',
                    ...
                    recommendation: 'Replace the literal with an environment reference (${env:NAME} for Cursor, ${NAME} for Claude Code), gitignore the real config, and rotate the exposed credential',
                    effort: 'low',
                    files: [candidate],
                });
            }
    ...
    async checkCursorIgnore() {
        ...
        if (content === null) {
            this.gaps.push({
                ...
                description: 'No .cursorignore file was found. Without it, Cursor may index generated and vendor trees on every retrieval.',
                impact: 'Agents pay tokens to read build output, caches, and dependencies',
                recommendation: 'Add a .cursorignore that excludes node_modules/, build output, virtualenvs, and engine-generated trees',
                effort: 'low',
                files: ['.cursorignore'],
            });
            return;
        ...
        if (missing.length >= 3) {
            this.gaps.push({
                ...
                recommendation: 'Add build/, .venv/, __pycache__/, vendor/, and other generated trees per the template .cursorignore',
                effort: 'low',
                files: ['.cursorignore'],
            });
        }
    }
    async readFileOrNull(relativePath) {
        try {
            return await fs_1.promises.readFile(path_1.default.join(this.rootDir, relativePath), 'utf8');
        }
        catch {
            return null;
        }
    }
    async findEnvSampleFile() {
    ...
    async fileExists(relativePath) {
        try {
            await fs_1.promises.access(path_1.default.join(this.rootDir, relativePath));
            return true;
        }
        catch {
            return false;
    ...
    applyManifestProfiles() {
        const manifest = this.stack?.manifest;
        if (!manifest) {
            return;
        }
        const manifestTechs = Array.isArray(manifest.technologies)
            ? manifest.technologies.map((tech) => tech.toLowerCase())
            : [];
        const packageManager = (manifest.packageManager || '').toLowerCase();
        if (manifestTechs.some((tech) => NODE_TECH_HINTS.includes(tech)) ||
            NODE_PACKAGE_MANAGERS.includes(packageManager)) {
            this.profiles.add('node');
        }
        if (manifestTechs.some((tech) => PYTHON_TECH_HINTS.includes(tech)) ||
    ...
    syncProfilesWithLanguageProfile() {
    ...
    hasTestingFramework(name) {
        const needle = name.toLowerCase();
        return (this.stack.tooling?.testing?.frameworks || []).some(framework => framework.name.toLowerCase() === needle);
    }
    generateReport() {
        // Sort gaps by severity
        const severityOrder = { high: 3, medium: 2, low: 1 };
        this.gaps.sort((a, b) => severityOrder[b.severity] - severityOrder[a.severity]);
        let report = '# Development Environment Gap Analysis Report\n\n';
        ...
        report += '---\n\n*Generated by Development Environment gap analyzer*';
        return report;
    }
    /**
    ...
    async saveReport(report) {
        const outputPath = path_1.default.join(this.rootDir, '.devenv', 'gaps-report.md');
        await fs_1.promises.mkdir(path_1.default.dirname(outputPath), { recursive: true });
        await fs_1.promises.writeFile(outputPath, report);
        logger.info(`Gap report saved to ${outputPath}`);
    }
    /** Write the structured report that the doctor consumes. */
    async saveJsonReport() {
        const outputPath = path_1.default.join(this.rootDir, '.devenv', 'gaps-report.json');
        await fs_1.promises.mkdir(path_1.default.dirname(outputPath), { recursive: true });
        await fs_1.promises.writeFile(outputPath, `${JSON.stringify(this.buildGapReport(), null, 2)}\n`);
