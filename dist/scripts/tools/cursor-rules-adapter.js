...
exports.RETIRED_RULE_REPLACEMENTS = exports.RETIRED_ALWAYS_ON_FILES = exports.STACK_SCOPED_FILES = void 0;
exports.detectExistingRules = detectExistingRules;
exports.shouldIncludeRule = shouldIncludeRule;
exports.adaptRulesForStack = adaptRulesForStack;
exports.getRuleSelection = getRuleSelection;
const fs_1 = require("fs");
const path_1 = __importDefault(require("path"));
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)({ context: 'cursor-rules-adapter' });
/**
 ...
 */
exports.RETIRED_ALWAYS_ON_FILES = [
...
exports.RETIRED_RULE_REPLACEMENTS = {
 ...
 */
async function detectExistingRules(projectRoot) {
    const rulesDir = path_1.default.join(projectRoot, '.cursor', 'rules');
    const result = {
        present: false,
        existingFiles: [],
        stackFiles: [],
    ...
    try {
        await fs_1.promises.access(rulesDir);
        result.present = true;
        const entries = await fs_1.promises.readdir(rulesDir, { withFileTypes: true });
        const mdcFiles = [];
        ...
        for (const file of mdcFiles) {
            if (exports.STACK_SCOPED_FILES.includes(file)) {
                result.stackFiles.push(file);
            }
            else if (exports.RETIRED_ALWAYS_ON_FILES.includes(file)) {
 ...
 */
function shouldIncludeRule(ruleFile, stackReport) {
    const techNames = stackReport.technologies.map(t => t.name.toLowerCase());
    const hasTypeScript = stackReport.quality.typescript || techNames.some(n => n.includes('typescript'));
    const hasJavaScript = techNames.some(n => n.includes('javascript') || n.includes('node.js') || n.includes('node'));
    const hasPython = techNames.some(n => n.includes('python') ||
        n.includes('pytest') ||
        n.includes('fastapi') ||
        n.includes('django') ||
        n.includes('flask'));
    const hasFrontend = techNames.some(n => n.includes('react') ||
        n.includes('next.js') ||
        n.includes('nextjs') ||
        n.includes('vue') ||
        n.includes('svelte')) || stackReport.frameworks.type !== 'vanilla';
    // Rule mapping
    switch (ruleFile) {
        ...
        case '15-shell-scripts.mdc':
            ...
            return (stackReport.files.key_patterns.some(p => p.includes('.sh') || p.includes('.ps1') || p.includes('.bat')) || true); // Include by default as many projects have scripts
 ...
 */
async function adaptRulesForStack(stackReport, templateRulesPath) {
    const selectedRules = [];
    try {
        // Get all available rule files from template
        const entries = await fs_1.promises.readdir(templateRulesPath, { withFileTypes: true });
        ...
        for (const stackFile of exports.STACK_SCOPED_FILES) {
 ...
 */
function getRuleSelection(stackReport, availableRules) {
    const stackRules = [];
    const skippedRules = [];
    const retiredRules = [];
    for (const rule of availableRules) {
        if (exports.STACK_SCOPED_FILES.includes(rule)) {
        ...
        else if (exports.RETIRED_ALWAYS_ON_FILES.includes(rule)) {
    ...
    if (retiredRules.length > 0) {
        reasons.push(`Found ${retiredRules.length} retired always-on rule(s); migrate to AGENTS.md and .agents/skills/`);
    }
    return {
        stackRules,
        skippedRules,
...
//# sourceMappingURL=cursor-rules-adapter.js.map