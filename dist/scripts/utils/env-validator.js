 ...
 * ```
 */
function requireEnvVar(name, options = {}) {
    ...
    if (options.validator) {
        ...
        if (!isValid) {
            const errorMessage = options.errorMessage || `Environment variable ${name} failed validation`;
 ...
 * ```
 */
function validateEnvVar(name, validator, errorMessage) {
    const value = process.env[name];
    if (!value) {
 ...
 * ```
 */
function requireEncryptionKey(name, expectedBytes = 32) {
    return requireEnvVar(name, {
        validator: value => (0, crypto_helpers_1.verifyKeyFormat)(value, expectedBytes),
 ...
 * ```
 */
function checkMissingEnvVars(vars) {
