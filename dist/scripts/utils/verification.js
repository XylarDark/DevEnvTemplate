...
exports.verifyPreCommit = verifyPreCommit;
exports.verifyPreDeployment = verifyPreDeployment;
...
const crypto_helpers_1 = require("./crypto-helpers");
/**
 ...
 * ```
 */
async function verifyPreCommit(projectRoot) {
    const errors = [];
    const warnings = [];
 ...
 * ```
 */
async function verifyPreDeployment(projectRoot) {
    const errors = [];
    const warnings = [];
    ...
    if (fs.existsSync(envExamplePath)) {
        ...
        const encryptionKeyVars = requiredVars.filter(v => v.toLowerCase().includes('key') ||
            v.toLowerCase().includes('secret') ||
            v.toLowerCase().includes('encryption'));
        for (const varName of encryptionKeyVars) {
            if (!(0, env_validator_1.hasEnvVar)(varName)) {
                ...
                if (value && value.length >= 40) {
                    ...
                    try {
                        if (!(0, crypto_helpers_1.verifyKeyFormat)(value, 32) && !(0, crypto_helpers_1.verifyKeyFormat)(value, 64)) {
 ...
 * ```
 */
async function verifyEnvironment(projectRoot) {
    const errors = [];
    const warnings = [];
    // Check for .env file
 ...
 * ```
 */
async function verifyAll(projectRoot, checks = [
    'preCommit',
    'preDeployment',
    'environment',
]) {
