...
exports.DEFAULT_HEALTH_SCORE_CONFIG = void 0;
exports.buildDoctorReport = buildDoctorReport;
exports.calculateHealthScore = calculateHealthScore;
exports.loadHealthScoreConfig = loadHealthScoreConfig;
 ...
 */
const DEFAULT_HEALTH_SCORE_CONFIG = {
...
exports.DEFAULT_HEALTH_SCORE_CONFIG = DEFAULT_HEALTH_SCORE_CONFIG;
/**
...
async function runDoctor(options = {}) {
    ...
    const reportDir = path_1.default.join(workingDir, '.devenv');
    const scanMode = options.mode === 'fast' ? 'fast' : 'full';
    ...
    const stackDetectorDistPath = path_1.default.join(__dirname, '../tools/stack-detector.js');
    const stackDetectorSourcePath = path_1.default.join(__dirname, '../../../scripts/tools/stack-detector.ts');
    const stackDetectorPath = (0, fs_1.existsSync)(stackDetectorDistPath)
        ? stackDetectorDistPath
        : (0, fs_1.existsSync)(stackDetectorSourcePath)
    ...
    try {
        ...
        const stackOutput = (0, child_process_1.execSync)(stackCommand, {
            cwd: workingDir,
            encoding: 'utf8',
    ...
    if (options.integrateCursorRules) {
        ...
        try {
            ...
            const possiblePaths = [
                path_1.default.join(workingDir, '.devenv', '.cursor', 'rules'),
                path_1.default.join(__dirname, '../../../.cursor/rules'),
            ];
    ...
    const gapAnalyzerPath = path_1.default.join(__dirname, '../tools/gap-analyzer.js');
    let gapReport;
    try {
        const gapArgs = [];
        if (scanMode === 'fast') {
        ...
        const gapCommand = gapArgs.length > 0
        ...
        (0, child_process_1.execSync)(gapCommand, {
            cwd: workingDir,
            encoding: 'utf8',
        ...
        const gapsJsonPath = path_1.default.join(reportDir, 'gaps-report.json');
        if (!(0, fs_1.existsSync)(gapsJsonPath)) {
            throw new Error(`${path_1.default.relative(workingDir, gapsJsonPath)} was not produced. ` +
                'Rebuild the tools (npm run build) so the gap analyzer emits structured output.');
        }
        gapReport = JSON.parse(await fs_1.promises.readFile(gapsJsonPath, 'utf8'));
        if (!Array.isArray(gapReport.gaps)) {
            throw new Error(`${path_1.default.relative(workingDir, gapsJsonPath)} has no 'gaps' array.`);
        }
    }
    catch (error) {
    ...
    const reportPath = path_1.default.join(reportDir, 'health-report.json');
    await fs_1.promises.writeFile(reportPath, JSON.stringify(report, null, 2));
    if (!options.json) {
        console.log(`\n💾 Full report saved: ${path_1.default.relative(workingDir, reportPath)}`);
    }
 ...
 * ignored.
 */
async function loadHealthScoreConfig(workingDir) {
    const candidatePaths = [
        path_1.default.join(workingDir, 'config', 'quality-budgets.json'),
        path_1.default.join(__dirname, '../../../config/quality-budgets.json'),
    ];
    for (const candidate of candidatePaths) {
        ...
        try {
            const parsed = JSON.parse(await fs_1.promises.readFile(candidate, 'utf8'));
            ...
            console.error(`   Warning: ignoring malformed healthScore config in ${candidate}: ${error.message}`);
        }
    }
 ...
 */
function buildDoctorReport(gapReport, config) {
    const critical = [];
    const warnings = [];
    const info = [];
    const quickWins = [];
    const unscoredCategories = new Set();
 ...
 * dimensions at once.
 */
function calculateHealthScore(gaps, config) {
    const dimensions = [
        'security',
        'quality',
        'testing',
    ...
    const penaltyByDimension = new Map(dimensions.map(dimension => [dimension, 0]));
    ...
    const scoreFor = (dimension) => Math.max(0, Math.min(100, 100 - penaltyByDimension.get(dimension)));
    const scores = {
...
function displayReport(report) {
    // Overall health
    const healthColor = report.healthScore.overall >= 80 ? '🟢' : report.healthScore.overall >= 60 ? '🟡' : '🔴';
...
const EFFORT_TO_ESTIMATE = {
    low: '< 10 min',
    medium: '~1 hour',
    high: '> 1 day',
};
function estimateFixTime(gap) {
 ...
 */
function createQuickWinContext(rootDir, stack, packageJson) {
    const resolve = (relativePath) => {
        const absolutePath = path_1.default.resolve(rootDir, relativePath);
        if (absolutePath !== rootDir && !absolutePath.startsWith(rootDir + path_1.default.sep)) {
    ...
    return {
        ...
        stack,
        packageJson,
        hasFile: async (relativePath) => (0, fs_1.existsSync)(resolve(relativePath)),
        readFile: (relativePath) => fs_1.promises.readFile(resolve(relativePath), 'utf8'),
        writeFile: async (relativePath, content) => {
            const absolutePath = resolve(relativePath);
            await fs_1.promises.mkdir(path_1.default.dirname(absolutePath), { recursive: true });
            await fs_1.promises.writeFile(absolutePath, content);
        },
        updateJson: async (relativePath, updater) => {
            const absolutePath = resolve(relativePath);
            const existing = (0, fs_1.existsSync)(absolutePath)
                ? JSON.parse(await fs_1.promises.readFile(absolutePath, 'utf8'))
                : {};
            await fs_1.promises.writeFile(absolutePath, `${JSON.stringify(updater(existing), null, 2)}\n`);
        },
    };
}
/**
 ...
 */
async function applyQuickFixes(workingDir, stackData, options) {
    const { getApplicableQuickWins } = await Promise.resolve().then(() => __importStar(require('./quick-wins')));
    const packageJsonPath = path_1.default.join(workingDir, 'package.json');
    let packageJson;
    if ((0, fs_1.existsSync)(packageJsonPath)) {
        try {
            packageJson = JSON.parse(await fs_1.promises.readFile(packageJsonPath, 'utf8'));
        }
        catch (error) {
...
async function resolveProjectRoot(cwd, override) {
    const envOverride = process.env.DEVENV_PROJECT_ROOT;
    const requested = override || envOverride;
    let candidate = requested ? path_1.default.resolve(cwd, requested) : cwd;
    // Use enhanced path resolver for better detection
    try {
        const resolved = (0, path_resolver_1.resolveProjectRoot)(candidate);
    ...
    if (!requested && path_1.default.basename(candidate) === '.devenv') {
