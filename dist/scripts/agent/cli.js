...
const logger = (0, logger_1.createLogger)({ context: 'agent-cli' });
class AgentCLI {
    rl;
    constructor() {
    ...
    async collectRequirements() {
        ...
        const coreFeatures = await this.promptMultiSelect('Which core features does your product need?', featureOptions, [] // no defaults
        );
    ...
    generateManifest(requirements) {
        ...
        if (requirements.needsCI)
            features.push('ci');
        if (requirements.needsDocker)
