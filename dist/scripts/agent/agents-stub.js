...
exports.buildAgentsStub = buildAgentsStub;
exports.writeAgentsStub = writeAgentsStub;
exports.looksLikeTemplateAgents = looksLikeTemplateAgents;
...
function projectName(projectRoot, stackReport) {
    const manifest = stackReport.manifest;
    if (manifest?.name && typeof manifest.name === 'string') {
        return manifest.name;
    }
    return path_1.default.basename(projectRoot);
}
function formatStackLines(stackReport) {
    const lines = [];
...
function formatCommandTable(stackReport) {
...
function formatLayoutLines(stackReport) {
    const lines = [];
...
function formatDeclinedLayers(declinedLayers) {
...
function formatAcceptedAbsences(absences) {
 ...
 */
function buildAgentsStub(options) {
 ...
 */
async function writeAgentsStub(options) {
    const target = path_1.default.join(options.projectRoot, 'AGENTS.md');
...
//# sourceMappingURL=agents-stub.js.map