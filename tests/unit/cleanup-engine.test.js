...
describe('CleanupEngine', () => {
  ...
  describe('Block Marker Detection', () => {
    ...
    test('should not modify files without template markers', async () => {
      ...
      assert.strictEqual(
        report.summary.blocksRemoved,
        0,
        'Should not remove any blocks from clean code'
      );
    });
  });
  ...
  describe('Report Generation', () => {
    test('should generate report with summary', async () => {
      ...
      assert.ok(
        typeof report.summary.totalActions === 'number',
        'Summary should have totalActions count'
      );
      ...
      try {
        ...
        if (report.actions.length > 0) {
          assert.strictEqual(
            exitCode,
            2,
            'Should exit with code 2 when actions detected in failOnActions mode'
          );
        }
      } finally {
        await cleanupTempFixture(tempDir);
