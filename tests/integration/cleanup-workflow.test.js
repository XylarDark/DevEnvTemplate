...
const { executeCleanup } = require('../../dist/scripts/cleanup/engine');
const {
  createTempFixture,
  cleanupTempFixture,
  fileExists,
  readFile,
} = require('../utils/fixture-helper');
...
describe('Cleanup Workflow Integration', () => {
  ...
  describe('Basic Node.js Project Cleanup', () => {
    ...
    test('should remove template-only blocks from files', async () => {
      ...
      assert.ok(
        originalContent.includes('TEMPLATE-ONLY:START'),
        'Original should have template markers'
      );
      ...
      assert.ok(
        !cleanedContent.includes('TEMPLATE-ONLY:START'),
        'Cleaned should not have template markers'
      );
      assert.ok(
        !cleanedContent.includes('templateOnlyFunction'),
        'Cleaned should not have template function'
      );
    });
  ...
  describe('Already Clean Project', () => {
    test('should not modify clean project', async () => {
      ...
      const basicConfigPath = path.join(
        __dirname,
        '..',
        'fixtures',
        'basic-node-project',
      ...
      assert.ok(
        report.summary.totalActions === 0 || report.summary.blocksRemoved === 0,
  ...
  describe('Dry Run Mode', () => {
    test('should not modify files in dry-run mode', async () => {
      ...
      assert.ok(
        report.actions.every(a => a.dryRun === true),
