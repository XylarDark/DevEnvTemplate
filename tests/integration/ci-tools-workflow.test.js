...
describe('CI Tools Workflow Integration', () => {
  ...
  before(async () => {
    ...
    await fs.writeFile(path.join(tempDir, 'src', 'index.js'), 'console.log("Hello World");');
  });
  ...
  it('should run stack-detector and generate stack report', async () => {
    ...
    const stackReportExists = await fs
      .access(stackReportPath)
      .then(() => true)
      .catch(() => false);
  ...
  it('should run gap-analyzer after stack-detector', async () => {
    ...
    assert.ok(
      gapsReport.includes('# Development Environment Gap Analysis Report'),
  ...
  it('should complete full CI workflow: stack-detector → gap-analyzer → plan-generator', async () => {
    ...
    await execAsync(`node "${stackDetectorPath}"`, { cwd: tempDir });
    const stackReportExists = await fs
      .access(path.join(devenvDir, 'stack-report.json'))
      .then(() => true)
      .catch(() => false);
    ...
    await execAsync(`node "${gapAnalyzerPath}"`, { cwd: tempDir });
    const gapsReportExists = await fs
      .access(path.join(devenvDir, 'gaps-report.md'))
      .then(() => true)
      .catch(() => false);
    ...
    await execAsync(`node "${planGeneratorPath}"`, { cwd: tempDir });
    const planExists = await fs
      .access(path.join(devenvDir, 'hardening-plan.md'))
      .then(() => true)
      .catch(() => false);
  ...
  it('should handle missing gaps report gracefully in plan-generator', async () => {
    ...
    try {
      ...
      assert.ok(
        error.message.includes('Gaps report not found'),
        'Should error about missing gaps report'
      );
    } finally {
      await fs.rm(cleanDir, { recursive: true, force: true });
    }
  ...
  it('should validate gap report structure is parseable by plan-generator', async () => {
    ...
    if (hasGaps) {
      ...
      assert.ok(
        gapsReport.includes('**Recommendation:**'),
        'Gaps should have Recommendation field'
      );
