...
import { executeCleanup } from './engine';
...
async function main(): Promise<void> {
  ...
  const options = program.opts<CleanupCliOptions>();
  ...
  if (concurrency !== undefined && (!Number.isInteger(concurrency) || concurrency < 1)) {
    console.error(
      `Invalid --concurrency value: ${options.concurrency}. Expected a positive integer.`
    );
    process.exit(1);
  }
